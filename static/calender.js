...
// 天気と画像データを結びつける
const icon = {hare:'hare.png', kumori:'kumori.png', kosame:'kosame.png', ame:'ame.png', ooame:'ooame.png', mizore:'mizore.png', yuki:'yuki.png', ooyuki:'ooyuki.png', kaminari:'kaminari.png', kaze:'kaze.png'}
        ...
        for (var j = 0; j < week.length; j++) {
            if (i == 0 && j < startDayOfWeek) {
                ...
                if (match.length !== 0
                    ...
                    if (date === chosen_date){
                        calendar += '<td id="tenki">' + `<button type="submit" name="date" id="chosen" value=${date}>` + count + `<br><img src="/static/tenki/${icon[match[0][1]]}" height="60px" width="60px" />` + '</button></td>';
                    } else if (date === latest) {
                        calendar += '<td id="tenki">' + `<button type="submit" name="date" id="latest2" value=${date}>` + count + `<br><img src="/static/tenki/${icon[match[0][1]]}" height="60px" width="60px" />` + '</button></td>';
                    } else if (date === oldest) {
                        calendar += '<td id="tenki">' + `<button type="submit" name="date" id="oldest2" value=${date}>` + count + `<br><img src="/static/tenki/${icon[match[0][1]]}" height="60px" width="60px" />` + '</button></td>';
                    } else {
                        calendar += '<td id="tenki">' + `<button type="submit" name="date" id="icon" value=${date}>` + count + `<br><img src="/static/tenki/${icon[match[0][1]]}" height="60px" width="60px" />` + '</button></td>';
                    }
                } else {
